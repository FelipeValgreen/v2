...
export function QuoteForm({leadWriteEnabled=false,powderCoatingEnabled=false}:{leadWriteEnabled?:boolean;powderCoatingEnabled?:boolean}){
 ...
 const [step,setStep]=useState<QuoteStep>(1);const [status,setStatus]=useState("");const [submitting,setSubmitting]=useState(false);const started=useRef(false);const formRef=useRef<HTMLFormElement>(null);
 ...
 return <form ref={formRef} className="quote-form rinon-quote-flow" onSubmit={submit} onFocusCapture={start}>
  ...
  <fieldset className="quote-step-panel wide" data-quote-step="2" hidden={step!==2}>
   ...
   <div className="quote-step-grid">
    ...
    <div className="wide quote-upload-shell" aria-label="Adjuntos todavía no habilitados"><div><b>Foto, plano o PDF</b><span>Puedes indicarnos que lo tienes y compartirlo por WhatsApp durante el seguimiento.</span></div><button type="button" disabled aria-disabled="true">Adjuntar archivo <small>próximamente</small></button></div>
   </div>
