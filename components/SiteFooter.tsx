...
export function SiteFooter(){
 ...
 return <footer className="s6-footer">
  <div className="container s6-footer-main s6-footer-main-v2">
   ...
   <nav className="s6-footer-column" aria-label="Servicios"><strong>Servicios</strong>{serviceNavItems.map(item=><Link key={item.href} href={item.href}>{item.label}</Link>)}<Link href={projectNavItem.href}>{projectNavItem.label}</Link></nav>
   <div className="s6-footer-column s6-footer-contact"><strong>RINON</strong><Link href={aboutNavItem.href}>{aboutNavItem.label}</Link><Link href="/empresas">Empresas</Link>{whatsapp?<a href={whatsapp} target="_blank" rel="noopener noreferrer" data-event="contact_whatsapp" data-cta-location="footer">WhatsApp ↗</a>:null}{phoneHref?<a href={phoneHref} data-event="contact_phone" data-cta-location="footer">{publicPhoneDisplay()}</a>:null}<span>{publicAddressDisplay()}</span><Link href="/nosotros#ubicacion">Cómo llegar ↗</Link><Link href="/contacto">Contacto</Link></div>
  </div>
