...
import {aboutNavItem,companyNavItem,productNavGroups,projectNavItem,serviceNavItems} from "@/lib/navigation";
import {whatsappUrl} from "@/lib/whatsapp";

export function MobileNav(){
 const [open,setOpen]=useState(false); const pathname=usePathname(); const toggleRef=useRef<HTMLButtonElement>(null); const panelRef=useRef<HTMLDivElement>(null); const whatsapp=whatsappUrl();
 ...
 return <div className="mobile-nav-wrap prd2-mobile-nav">
  ...
  {open?<div ref={panelRef} role="dialog" aria-modal="true" aria-label="Navegación" className="mobile-nav-panel" id="mobile-navigation" onKeyDown={trapFocus}>
   ...
   <nav className="mobile-nav-primary" aria-label="Navegación móvil principal">
    <Link href={projectNavItem.href} onClick={()=>setOpen(false)}><span>01</span>{projectNavItem.label}</Link>
    <Link href={companyNavItem.href} onClick={()=>setOpen(false)}><span>02</span>{companyNavItem.label}</Link>
   </nav>
   ...
   <nav className="mobile-nav-primary mobile-nav-secondary" aria-label="Información de RINON"><Link href={aboutNavItem.href} onClick={()=>setOpen(false)}><span>03</span>{aboutNavItem.label}</Link></nav>
