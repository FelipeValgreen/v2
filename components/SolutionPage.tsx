...
export function SolutionPage({ solution }: { solution: Solution }) {
  ...
  return <main className={`prd2-solution-page ${visualTheme}`}>
    ...
    <section id="cotizar" className="prd2-final" data-reveal><div className="container"><div><span className="prd2-kicker">SIGUIENTE PASO</span><h2>{launchEnabled?"Cuéntanos qué necesitas fabricar.":"Consulta el estado de esta capacidad."}</h2><p>{launchEnabled?"Incluye cantidad, ubicación y el mejor antecedente disponible. Si faltan datos, pediremos solo los relevantes para evaluar.":"La ruta existe para revisión, pero el servicio no se publica como disponible hasta cerrar sus límites operativos."}</p></div><div className="prd2-final-actions"><Link className="prd2-btn primary" data-event={launchEnabled?"quote_start":undefined} data-cta-location="solution_footer" href={quoteHref}>{quoteLabel} ↗</Link>{launchEnabled?<WhatsAppCTA category={solution.quoteCategory} location="solution_footer" label="Hablar por WhatsApp" className="prd2-btn ghost" />:null}</div></div></section>
  </main>;
}
