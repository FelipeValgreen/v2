...
if(!cutoverAuthorized){for(const name of productionFlags)check(!envTrue(name),`${name} is not active before cutover authorization`);check(!envTrue("RINON_URL_INVENTORY_COMPLETE"),"URL inventory remains explicitly incomplete before final crawl/GSC reconciliation");notes.push("Cutover is intentionally blocked. Set RINON_CUTOVER_AUTHORIZED=true only during an explicitly authorized production release.");notes.push("RINON_URL_INVENTORY_COMPLETE must remain false until the live crawl and Search Console export are reconciled.")}else{for(const name of productionFlags)check(envTrue(name),`${name}=true for authorized production cutover`);check(envTrue("RINON_URL_INVENTORY_COMPLETE"),"full live URL inventory + GSC reconciliation approved for cutover");check(envTrue("RINON_LEGAL_APPROVED"),"legal identity/content approved for production");check(Boolean(process.env.SUPABASE_URL),"SUPABASE_URL is present for production administration/read access");check(Boolean(process.env.SUPABASE_SERVICE_ROLE_KEY),"SUPABASE_SERVICE_ROLE_KEY is present for production administration/read access");check(publicIntake.includes(".supabase.co/functions/v1/rinon-public-intake"),"production public intake endpoint remains configured")}
