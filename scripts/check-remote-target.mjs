...
const base = (process.env.RINON_REMOTE_BASE_URL || process.env.RINON_PLAYWRIGHT_BASE_URL || "").trim();

if (!base) {
  console.error("RINON REMOTE BROWSER QA: falta RINON_REMOTE_BASE_URL.");
  console.error("Pasa explícitamente la URL del deployment a validar, no un alias:");
  console.error("  vercel ls rinon-v2");
  console.error("  RINON_REMOTE_BASE_URL=https://<deployment>.vercel.app npm run qa:browser:remote");
  console.error("Alias compatible: RINON_PLAYWRIGHT_BASE_URL.");
...
try {
  ...
  console.error(`RINON REMOTE BROWSER QA: RINON_REMOTE_BASE_URL inválida (${base}): ${error.message}`);
  process.exit(1);
}
