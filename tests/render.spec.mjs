...
async function assertBrandChrome(page){
 ...
 expect(chrome.logoSrc).toContain("/brand/logo-rinon-horizontal-white.png");
...
test("home renders RC7 brand, navigation and quality-gated structure visual",async({page})=>{
 ...
 await page.getByRole("button",{name:/Productos/}).click();await expect(page.locator("#mega-products")).toBeVisible();await expect(page.getByRole("link",{name:/Camas balinesas/}).first()).toBeVisible();await expect(page.getByText("Cierres y accesos")).toBeVisible();
 await expect(page.getByRole("link",{name:"Proyectos a medida"}).first()).toBeVisible();
 await page.getByRole("button",{name:/Servicios/}).click();await expect(page.locator("#mega-services")).toBeVisible();await expect(page.getByRole("link",{name:/Pintura electrostática/}).first()).toBeVisible();await expect(page.locator("#mega-services")).not.toContainText("Fabricación a medida");
