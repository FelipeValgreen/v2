...
- Production schema asset alignment at cutover: `https://rinon.cl/brand/logo-rinon-horizontal.png` currently returns HTTP 404 on the live site because `rinon.cl` still serves the old production site. The same asset returns 200 on the RC.7 preview, so this is not a code bug. Keep the Organization JSON-LD pointed at the canonical production URL, and verify that this path returns 200 on production immediately after the authorized cutover, just as canonical and `og:url` align to `rinon.cl`.
