...
export const metadata = routeMetadata("/recursos","Recursos de fabricación metálica","Guías prácticas para preparar cotizaciones y requerimientos de fabricación metálica, estructuras, cierres y camarotes.");
...
export default function Page(){return <main className="v5-editorial-page">
  ...
  <section className="v2-solution-section"><div className="container"><div className="v2-resource-grid">{publicResourceArticles.map((article)=><Link key={article.slug} href={`/recursos/${article.slug}`}><span>{article.category}</span><h3>{article.title}</h3><p>{article.description}</p><b>Leer guía →</b></Link>)}</div></div></section>
